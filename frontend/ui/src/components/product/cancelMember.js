...
class CancelMember extends React.Component {
  ...
  handleOk = () => {
    if (
      this.state.user_id === this.state.my_id &&
      this.props.my_role === 'Product Owner'
    ) {
  ...
  render () {
    const disableButton = this.props.my_role === 'Product Owner'
      ? this.props.user_role === 'Product Owner' // Product Owner cannot remove himself
      : this.props.user_id !== this.props.my_id; // Developer/Scrum Master cannot remove anyone but himself
    return (
      <div>
